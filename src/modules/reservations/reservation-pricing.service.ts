import {
  ...
  export class ReservationPricingService {
    ...
    async calculate(dto: QuoteDto) {
      ...
      const inapamVisitors = Math.min(dto.inapamVisitors ?? 0, payableAdults);
      ...
      const snapshotDescription =
        campaignResult.contentLayer.description ?? translation?.description ?? null;
  
      const snapshotIncludes =
        campaignResult.contentLayer.includes ??
        translation?.includes ??
        [];
  
      const snapshotExcludes =
        campaignResult.contentLayer.excludes ??
        translation?.excludes ??
        [];
  
      const snapshotNotes =
        campaignResult.contentLayer.notes ??
        translation?.notes ??
        [];
      ...
      return {
        ...
        packageSummary: {
          ...
          coverMedia:
            campaignResult.contentLayer.image ?? pkg.coverMedia ?? null,
        },
        passengers: {
          adults: dto.adults,
          children: dto.children,
